function [net, info] = cnn_ucf_slurm1(varargin)
...
% 
parpool(12)
addpath(genpath('/triton/ics/project/imagedb/anwerr1/matconvnet-1.0-beta18/'));

run(fullfile(fileparts(mfilename('fullpath')), ...
  '..', '..', 'matlab', 'vl_setupnn.m')) ;

% opts.dataDir = fullfile(vl_rootnn, 'data','ILSVRC2012') ; % original line
%%  A General Directory location of Imagenet 2012 datasets images 
opts.dataDir = fullfile('/triton/ics/project/imagedb/anwerr1/Datasets/','ucf_sports_actions') ;
%% 
expDir = '/triton/ics/project/imagedb/anwerr1/matconvnet-1.0-beta18/examples/ucf_sports/';
...
opts.expDir = fullfile(expDir,'data', sprintf('ucfsports-%s', sfx)) ;
...
net = cnn_ucf_init('model', opts.modelType, ...
...
if exist(opts.imdbPath)
  ...
  imdb = cnn_ucf_setup_data('dataDir', opts.dataDir, 'lite', opts.lite) ;
  mkdir(opts.expDir) ;
