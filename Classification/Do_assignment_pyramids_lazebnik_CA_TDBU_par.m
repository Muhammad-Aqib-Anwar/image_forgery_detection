function All_hist=Do_assignment_pyramids_lazebnik_CA_TDBU_par(opts,assignment_opts,start_images, end_images,vocabulary,Pcw)
...
index_list1=getfield(load([opts.data_assignmentpath,'/',assignment_opts.index_name2]),'index_list');
patch_im_all=getfield(load([opts.data_assignmentpath,'/',assignment_opts.saliency_patches]),'patch_saliency_all');
...
for ii=start_images:end_images
    ...
    index_color=index_list1{ii};
    patch_im=patch_im_all{ii};
    patch_im=patch_im.^(assignment_opts.mu*(1-assignment_opts.alpha));
    ...
    pyramid_cell{1} = zeros(binsHigh, binsHigh, vocabulary_size*nClasses);
    ...
    for i=1:binsHigh
        for j=1:binsHigh
            ...
            %% Additional code to incorporate CA
            texton_patch_color = index_color((points(:,1) > x_lo) & (points(:,1) <= x_hi) & ...
                (points(:,2) > y_lo) & (points(:,2) <= y_hi));
            
            tt=zeros(vocabulary_size,nClasses);
            all_hist_combine=zeros(vocabulary_size*nClasses,1);
            for jkl=1:vocabulary_size
                 if size(imageread, 3) == 1
                      cw=texton_patch_color(texton_patch==jkl);
                      tt(jkl,:)=sum(Pcw(cw,:),1);
                 else
                      cw=texton_patch_color(texton_patch==jkl);
                      indx_tempt{1}=texton_patch;
                      aaa=indx_tempt{1}==jkl; 
                      sal_ind=find(aaa>0);             %%% extra line for bottom-up
                      sal_val=patch_im(sal_ind,:);     %%% extra line for bottom-up
                      Pcp12=Pcw(cw,:);                 %%% Conventional top-down
                      Pcp1=diag(sal_val)*Pcp12;        %%% for combining top-down and bottom-up
                      tt(jkl,:)=sum(Pcp1,1);     
                 end
                                  
            end
            %all_hist_combine(:,1)=reshape(tt,vocabulary_size*opts.nclasses,1);
            %all_hist_combine=normalize(all_hist_combine);
            pyramid_cell{1}(i,j,:) = reshape(tt,vocabulary_size*nClasses,1)/length(index);
    ...
    for l = 2:pyramidLevels
        pyramid_cell{l} = zeros(num_bins, num_bins, vocabulary_size*nClasses);
        for i=1:num_bins
            for j=1:num_bins
                pyramid_cell{l}(i,j,:) = ...
