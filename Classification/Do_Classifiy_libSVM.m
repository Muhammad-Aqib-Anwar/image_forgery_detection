...
%      size(histogram)
%      histogram = compute_mapping(histogram', 'SNE', 500);
%      histogram=histogram';
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%       histogram=[(0.75).*histogram1;(0.15).*histogram2;(0.7).*histogram3;(0.05).*histogram4];
%     histogram2=histogram2(1:12800,:); histogram2=histogram2./0.5;
%     histogram2=histogram2(1:12800,:); histogram2=histogram2./0.5;
%             histogram=[histogram1;histogram2];
%   histogram=[(0.6).*histogram1;(0.2).*histogram2;(0.2).*histogram3;(0.25).*histogram4];
% histogram=[(0.6).*histogram1;(0.35).*histogram2;(0.1).*histogram3;(0.05).*histogram4;(0.01).*histogram5];
...
%          histogram=[(0.3).*histogram1;(0.5).*histogram3;(0.4).*histogram2;(0.1).*histogram4];
%%%% This one 
%      histogram=[(0.1).*histogram1;(0.33).*histogram2;(0.47).*histogram3;(0.4).*histogram4]; 

%          histogram=[(0.35).*histogram1;(0.75).*histogram2;(0.15).*histogram3;(0.15).*histogram4];
...
%      histogram=[(0.8).*histogram1;(0.5).*histogram2;(0.45).*histogram3;(0.5).*histogram4;(0.65).*histogram6;(0.05).*histogram5;(0.1).*histogram7]; %% OT 
...
%                 [best_C,best_G,best_CV]=Do_CV_libSVM_chisq(histogram_train',train_truth);
               ...
               %%% uncomment next line for intersection kernel %%%
                   [predict_labels,svm_score, dec_values]=Do_libSVM_fast_multi(histogram_train',histogram_test',train_truth,test_truth,best_C);
...
%       err = avg_class_err(test_truth,predict_labels);

          %%% for computing the product of different kernel responses
%           save -v7.3 '/home/fahad/Datasets/flower_102/Data/Global/predict_labels_CNLBPSIFT_class1' predict_labels
%           save -v7.3 '/home/fahad/Datasets/flower_102/Data/Global/dec_values_CNLBPSIFT_class1' dec_values

%           save -v7.3 '/share/CIC/fahad/Caltech_101/Data/Global/predict_labels_PSIFT_1000' predict_labels
%           save -v7.3 '/share/CIC/fahad/Caltech_101/Data/Global/dec_values_PSIFT_1000' dec_values
       case 2   
