function [svm_score,best_CV]=Do_Classifiy_libSVM_Standford_test2(opts,classification_opts,rng)
VOCINIT2010_Action
% if nargin<2
%     classification_opts=[];
% end
...
% histogram=1./max(1e-12, sqrt(sum(histogram.^2)));
% histogram=histogram/sqrt(norm(histogram)^2+.001);
histogram1=sign(histogram1).*sqrt(abs(histogram1));
histogram2=sign(histogram2).*sqrt(abs(histogram2));
...
% histogram=[(0.7).*histogram1;(0.7).*histogram2;(0.3).*histogram3;(0.3).*histogram4];
% histogram=[normalize(histogram2,1);normalize(histogram1,1);normalize(histogram3,1)];
histogram=[(histogram1);histogram2];
% clear histogram1; clear histogram2; clear histogram3; clear histogram4

% histogram=histogram(1:24576,:);
% histogram=normalize(histogram,1);
