function []=Do_Detection_Pascal2007_ESS(opts,classification_opts)
% VOCINIT2007
...
if(classification_opts.trainmodel)    
 display('%%% STARTING TO LAERN THE INITIAL MODEL: %%%');
 best_C=200;
 histogram=getfield(load([opts.data_assignmentpath,'/',classification_opts.trainhist]),'All_hist');
 size(histogram)

 train_truth=getfield(load([opts.data_assignmentpath,'/',classification_opts.traintruth]),'train_truth');
 [model,weights]=Do_Model_train_Det_ESS(histogram',train_truth,best_C,opts,classification_opts);
 display('%%% The Initial Model is Computed %%%');
  pause
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Initial Model is Trained %%%%%%%%%%%%%%%%%%%%%%%%%%%% 

%% ESS Based Detection based on the Training Model 
%%%%%%%%%%%%%%% Load the trained model %%%%%%%%
weights=getfield(load([opts.data_globalpath,'/',classification_opts.cls_name,'_weights']),'weights');
model=getfield(load([opts.data_globalpath,'/',classification_opts.cls_name,'_model']),'model');

%%%%%%%%%%%%%%%%% Do the real ESS function %%%%%%%%%%%%%%
if(classification_opts.compute_ESS_flag)
    settings.levels=1;
    settings.number_det_test=1;
%      Do_Detection_ESS(opts, classification_opts,settings,weights)

%        Do_Detection_ESS_Original_par_Toons(opts, classification_opts,settings,weights)
Do_Detection_ESS_Original_par(opts, classification_opts,settings,weights)
end

%%%%%%%%%%%%%%%%%% Some Parameters for preprocessing %%%%%%%%%%%%%%%
if(classification_opts.preprocessing)
    display('%%% STARTING PREPROCESSING THE ESS BASED DETECTION: %%%');
    pause
    counter=1;
    for i =1:length(labels)  %%%% for val as testset , there are 5011 for VOC 2007 set
        i
    %           if(testset(i)==1)
                  points=getfield(load([data_locations{i},'/',classification_opts.index_name]),'assignment');
                  clst=[points(:,1), points(:,2), points(:,3)-1];  % 1 is subtracted from cluster ID since ESS assumes ID from 0 till n-1
    %               save_clst(strcat(opts.clstfolder,num2string(ii+size(ids_train,1),6),'.clst'), clst);

                  save_clst(strcat(data_locations{i},'/',classification_opts.index_name,'.clst'), clst);

                  %%%%%%% To create a image_size file %%%%%%%%%
    %              imt=sprintf('%s/%s',opts.imgpath,image_names{i});
    %              im=imread(imt);
    %              [h w ch]=size(im);
    %              image_size{counter}=[h w];
    %              counter=counter+1;
    %           end
...
if(classification_opts.drawAP)
    [recall,prec,ap]=VOCevaldet(opts,classification_opts.cls_name,true);
    ap
%     pause
end

% Do_Detection(opts, ESS_settings, ids_test, cls);
% display('size file made');
