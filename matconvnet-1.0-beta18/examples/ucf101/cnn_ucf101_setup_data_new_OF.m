function imdb = cnn_ucf101_setup_data_new_OF(opts,split_ind)
...
imdb.imageDir = fullfile(opts.dataDir, 'OpticalFlowImages') ;
% -------------------------------------------------------------------------
%                                                           Training images
% -------------------------------------------------------------------------
...
parfor i = 1:length(video_class)
    i
    ims=  dir(fullfile(opts.dataDir, 'OpticalFlowImages', video_class{i}(1:end-4),filesep,'*.jpg'));
    names{i} = strcat([video_class{i}(1:end-4), filesep], {ims.name}) ;
...
parfor i = 1:length(video_class_val)
    i
    ims=  dir(fullfile(opts.dataDir, 'OpticalFlowImages', video_class_val{i}(1:end-4),filesep,'*.jpg'));
